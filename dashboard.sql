...
final_result AS (
  SELECT 
	...
	COALESCE(ROUND(dr.total_spent/nullif(rc.distinct_visitor_ids, 0), 2),0) as cpu,
	COALESCE(ROUND(dr.total_spent/nullif(dlc.distinct_lead_count, 0), 2), 0) as cpl,
	coalesce(ROUND(dr.total_spent/nullif(sc.status_142_count, 0), 2), 0) as cppu,
	COALESCE(ROUND((pc.max_purchase_value - dr.total_spent) / NULLIF(dr.total_spent, 0), 2), 0) as roi
  FROM 
  ...
  SELECT 
    ...
    dr.total_spent AS total_cost,
    dlc.distinct_lead_count as leads_count,
    sc.status_142_count as purchases_count,
    pc.max_purchase_value as revenue,
	COALESCE(ROUND(dr.total_spent/nullif(rc.distinct_visitor_ids, 0), 2),0) as cpu,
	COALESCE(ROUND(dr.total_spent/nullif(dlc.distinct_lead_count, 0), 2), 0) as cpl,
	coalesce(ROUND(dr.total_spent/nullif(sc.status_142_count, 0), 2), 0) as cppu,
